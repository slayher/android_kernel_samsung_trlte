...
extern void mdss_dsi_dln0_phy_err(struct mdss_dsi_ctrl_pdata *ctrl);
	...
	int enable)
{
	...
	if (enable && !ctrl->ulps) {
		...
		MIPI_OUTP(ctrl->mmss_misc_io.base + 0x14, 0x3FF);
		MIPI_OUTP(ctrl->mmss_misc_io.base + 0x14, 0x83FF);
		...
		*/
		...
		MIPI_OUTP(ctrl->mmss_misc_io.base + 0x14, 0x3FF);
		MIPI_OUTP(ctrl->mmss_misc_io.base + 0x14, 0x0);
		usleep(100);
